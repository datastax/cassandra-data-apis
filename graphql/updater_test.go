...
func TestSchemaUpdater_Update(t *testing.T) {
	...
	updater, err := NewUpdater(schemaGen, "store", 10*time.Second, 30*time.Second, log.NewZapLogger(zap.NewExample()))
...
func TestSchemaUpdater_Expire(t *testing.T) {
	sessionMock := db.NewSessionMock()
	schemaGen := NewSchemaGenerator(db.NewDbWithSession(sessionMock), config.NewConfigMock().Default())

	sessionMock.
		SetSchemaVersion("a78bc282-aff7-4c2a-8f23-4ce3584adbb0").
		Times(3) // Called by `NewUpdater()` and two calls to `updater.update()`
	...
	updater, err := NewUpdater(schemaGen, "store", 10*time.Second, 100*time.Millisecond, log.NewZapLogger(zap.NewExample()))
	assert.NoError(t, err, "unable to create updater")
